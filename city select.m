...
%找出属于该元胞范围的城市数量【ps:这一步是在整个北京连同周围的7个被选入元胞最大半径的圆在内的城市，
%关于只选入北京元胞的城市还没有做！！！】
weathercity = 'C:/DELL/research/天气爬虫/其他站点的4月到5月26日/current_wea/2017-04-12.csv';
loncity=xlsread(weathercity,'G1:G12364');
latcity=xlsread(weathercity,'F1:F12364');
[xcity,ycity]=m_ll2xy(loncity,latcity);
lag=[1:12364];
label2=[];
xcircle=[];
ycircle=[];
k1=0;
for i=1:12364
        d1=power(xstation(a1)-xcity(i),2)+power(ystation(a1)-ycity(i),2);
        max(s2);
        if d1<=max(s2)&&d1>=0.0000001
            k1=k1+1;
            label2(k1)=i;
            xcircle(k1)=loncity(i);
            ycircle(k1)=latcity(i);
        end
end
